    ...
    function ($compile, clipboardDirectiveIdService, converterService, clipboardSwf, clipboardExpressInstallSwf) {
      ...
      window.clipboard = {
        ...
        copied: function (id) {
          ...
          if (scope.onCopied) { scope.onCopied(el.parents('.btn-clipboard')) }
        }
      }
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          if (typeof swfobject === 'undefined') { throw new Error('swfobject is required') }

          var flashObj, flashvars, attributes
          ...
          scope.$watch('data', function (newValue) {
            if (typeof newValue !== 'undefined' && element.setClipboardData) { element.setClipboardData(newValue) }
          })

          // check if flash plugin has been blocked
          setTimeout(function () {
            if (window.clipboard.ready) { return }
